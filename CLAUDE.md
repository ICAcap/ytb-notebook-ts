...
### Shared Components
- `components/sidebar.tsx` — Navigation sidebar with collapse/expand, theme toggle (light: nord, dark: abyss via DaisyUI)
...
- `src/app/themeProvider.tsx` — `ThemeProviders` wrapper using `next-themes`; sets `data-theme` attribute (light: "nord", dark: "abyss"); prevents theme flash on load
...
- **Theme System**: `next-themes` via `ThemeProviders` (`src/app/themeProvider.tsx`); `data-theme` attribute on `<html>` (light: "nord", dark: "abyss"); prevents flash on load
